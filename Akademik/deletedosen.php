...
if(isset($_GET['id'])){
	...
	if(mysqli_num_rows($cek) > 0){
		...
		if($del){
			...
			echo '<script>alert("Gagal menghapus data."); document.location="dosen.php";</script>';
		}
	}else{
		echo '<script>alert("ID tidak ditemukan di database."); document.location="dosen.php";</script>';
	}
}else{
	echo '<script>alert("ID tidak ditemukan di database."); document.location="dosen.php";</script>';
}
